...
public class OrderService {
    ...
    public OrderResponse createOrder(CreateOrderRequest request, String email) {
        ...
        BigDecimal roundUpAmount = request.getRoundUpAmount() != null ? BigDecimal.valueOf(request.getRoundUpAmount())
                : BigDecimal.ZERO;
        ...
        if ("WALLET".equalsIgnoreCase(request.getPaymentMethod())) {
            ...
            // Bakiyeden düş (Toplam ödemeyi düşüyoruz, ama birazdan yuvarlamayı geri
            // ekleyeceğiz)
        ...
        if (roundUpAmount.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal currentIyilikBalance = buyer.getIyilikBalance() != null ? buyer.getIyilikBalance()
                    : BigDecimal.ZERO;
            buyer.setIyilikBalance(currentIyilikBalance.add(roundUpAmount));
        }
